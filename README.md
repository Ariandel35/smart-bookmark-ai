...
| Progress feedback | The popup shows elapsed time and, after real progress is available, an estimated remaining time. It also surfaces if the background status has not changed for 45 seconds, while a lightweight once-per-second clock keeps time text fresh without reloading the full popup state every second. |
