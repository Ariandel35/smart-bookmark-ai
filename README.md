...
| Progress feedback | The popup shows elapsed time and, after real progress is available, an estimated remaining time. If the background status has not changed for 45 seconds, the detail panel explains that the model may still be slow and suggests waiting or cancelling and retrying with Fast mode. A lightweight once-per-second clock keeps time text fresh without reloading the full popup state every second. |
