...
| 设置补全 | 缺少服务商、Base URL 或模型时，会直接引导到设置页；只有本地规则处理后仍有书签确实需要模型时才要求 API 访问。 |
...
| 速度模式 | 快速模式跳过失效链接检测和额外目录规划请求，先走自定义规则、缓存复用和内置域名规则，剩余书签才进入 AI；完整模式保留链接检查和全局规划。 |
| 本地复跑 | 如果自定义规则、分类缓存和内置域名规则已覆盖全部书签，Marko 可以不要求 API Key 或模型接口授权就生成预览。 |
...
- 只有生成预览或已开启自动整理且本地规则、缓存无法覆盖时，书签元数据才会发送给你配置的模型服务商。
