...
| 慢模型 | DeepSeek 会在第一次请求前自动使用更安全的运行批大小；若仍超时，再继续自动降批重试。 |
| 应用提速 | 已生成的预览会复用保存的方案，本地重建书签，不会再次跑模型。 |
| 速度模式 | 快速模式跳过失效链接检测，让预览更快；完整模式会在分类前检查链接。 |
| DeepSeek | 新配置和重置配置默认更小；旧的大批量设置也会在本次运行中自动压低，减少 90 秒卡顿。 |
...
- 完整链接检查会直接访问书签对应的网站；快速模式会跳过这些检测。
