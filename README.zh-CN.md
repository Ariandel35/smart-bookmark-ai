...
| 慢模型 | DeepSeek 会使用更小的运行批量、请求前再次拆分、更短的服务商专属超时、压缩后的模型输入和更紧的输出预算；若仍超时，会临时拆到 1 条小批次继续重试。 |
...
| DeepSeek | 新配置和重置配置默认从 5 开始；旧的大批量设置会在本次运行中自动压低，并在每次模型请求前再次拆分，慢重试会继续拆小且不改写已保存设置。 |
