...
5. 快速模式使用本地规则；完整模式先规划全局目录，再用 AI 分批分类。
6. 最后一次性重建书签栏根目录。
...
| 慢模型 | DeepSeek 和 DeepSeek 兼容接口会跳过单独目录规划请求，运行批次最多 15 条，单个模型请求最多 5 条，并最多 3 个小请求并发处理；如果模型仍然超时，Marko 会停止等待本轮模型，改用本地规则、缓存、内置规则和待手动分类兜底完成。 |
...
- 完整模式会直接访问书签对应的网站做链接检查，并增加单独目录规划和模型分类请求；快速模式会跳过这些外部请求。
