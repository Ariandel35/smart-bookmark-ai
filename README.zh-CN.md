...
| 慢模型 | DeepSeek 和 DeepSeek 兼容接口运行批次最多 15 条，单个模型请求最多 5 条，并最多 3 个小请求并发处理；同时保留更短的服务商专属超时、压缩后的模型输入、更紧的输出预算和自适应小块重试：已完成的小请求会保留，只把失败小块继续拆到 1 条。 |
| 应用提速 | 已生成的预览会复用保存的方案，本地重建书签，不会再次跑模型。 |
| 速度模式 | 快速模式跳过失效链接检测、额外目录规划请求和模型等待；未命中本地规则的书签进入待手动分类。完整模式保留链接检查、全局规划和 AI 分类。 |
