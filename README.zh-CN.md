...
| 每批分类都临时生成文件夹 | 快速模式使用稳定根目录；完整模式在较快服务商上可先做全局规划 |
...
5. 快速模式使用本地规则；完整模式用 AI 分批分类。较快服务商可先规划全局目录；慢模型会跳过额外请求，并可在超时后本地兜底。
6. 最后一次性重建书签栏根目录。
...
| 弹窗模式切换 | 可直接在弹窗切换快速/完整模式；模式变化后已保存预览会自动失效。 |
| 设置补全 | 缺少服务商、Base URL 或模型时，会直接引导到设置页；快速预览不要求模型访问，完整模式只有在未缓存书签需要 AI 分类时才要求 API 访问。 |
| API 设置 | `测试并保存` 会验证连接，并保存当前可用配置。 |
| 慢模型 | DeepSeek 和 DeepSeek 兼容接口会跳过单独目录规划请求，运行批次最多 15 条，单个模型请求最多 5 条，并最多 3 个小请求并发处理；如果 10 秒无首包或 30 秒未完整返回，Marko 会停止等待本轮模型，改用本地规则、缓存、内置规则和待手动分类兜底完成。 |
| 应用提速 | 已生成的预览会复用保存的方案，本地重建书签，不会再次跑模型。 |
| 速度模式 | 快速模式跳过失效链接检测、额外目录规划请求和模型等待；未命中本地规则的书签进入待手动分类。完整模式保留链接检查和 AI 分类；慢模型会跳过额外目录规划请求，并可在超时后本地兜底。 |
| 本地复跑 | 快速模式可以不要求 API Key 或模型接口授权就生成预览，因为自定义规则、分类缓存、内置域名规则和待手动分类兜底会在本地完成。 |
| 自动整理 | 快速自动整理可以不填 API Key 本地运行；完整自动整理仍需要模型凭据和网站访问权限。 |
| DeepSeek | 新配置和重置配置默认从 15 开始；旧的大批量设置会在本次运行中自动压低，DeepSeek 兼容 Base URL 或模型名也会使用同一策略，旧的运行中任务会在下一批开始前自动规范化，且不改写已保存设置。 |
...
- 只有完整模式预览或已开启自动整理且本地规则、缓存无法覆盖时，书签元数据才会发送给你配置的模型服务商。
- 应用已保存的预览方案会直接本地重建，不会再次请求模型。
- API Key 只保存在本地，不会发送给本项目或开发者。
- 完整模式会直接访问书签对应的网站做链接检查，并使用模型分类；较快服务商可能额外请求目录规划，慢模型会跳过这一步。快速模式会跳过这些外部请求。
