...
| 慢模型 | DeepSeek 运行批次最多 10 条，单个模型请求最多 5 条，并以受控小并发处理；同时保留更短的服务商专属超时、压缩后的模型输入、更紧的输出预算和自适应小块重试：已完成的小请求会保留，只把失败小块继续拆到 1 条。 |
...
| DeepSeek | 新配置和重置配置默认从 10 开始；旧的大批量设置会在本次运行中自动压低，并在每次模型请求前再次拆分，以受控小并发处理，且不改写已保存设置。 |
