...
| 慢模型 | DeepSeek 和 DeepSeek 兼容接口会跳过单独目录规划请求，运行批次最多 12 条，单个模型请求最多 4 条，并最多 3 个小请求并发处理；如果 8 秒无首包或 18 秒未完整返回，Marko 会停止等待本轮模型，改用本地规则、缓存、内置规则和待手动分类兜底完成。 |
...
| DeepSeek | 新配置和重置配置默认从 12 开始；旧的大批量设置会在本次运行中自动压低，DeepSeek 兼容 Base URL 或模型名也会使用同一策略，旧的运行中任务会在下一批开始前自动规范化，且不改写已保存设置。 |
