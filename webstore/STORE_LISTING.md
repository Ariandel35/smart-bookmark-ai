...
- DeepSeek 等慢模型会先使用更小的运行批大小、更短的请求超时和更短的内置请求提示；响应过慢时还会自动降低当前批大小并重试
...
- Slow providers such as DeepSeek start with a smaller runtime batch size, shorter request timeouts, and shorter built-in prompts, then retry with smaller batches if needed
