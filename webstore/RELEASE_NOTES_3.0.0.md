...
3.0.0 优化了核心使用路径：先预览，再应用方案。应用预览不会再次跑模型；快速模式默认更快且权限更少；设置页使用内联确认和校验提示，API 检测成功后会保存连接配置，DeepSeek 等慢模型会先使用更安全批量并在超时后自动降批重试。
...
3.0.0 streamlines the core workflow: preview first, then apply the plan without a second model run. Fast mode skips dead-link checks for quicker runs and fewer permissions, while Complete mode keeps link checks. Settings now use inline confirmations and validation feedback, successful API tests save the connection, and slow providers such as DeepSeek start with safer runtime batches before timeout retries shrink again.
