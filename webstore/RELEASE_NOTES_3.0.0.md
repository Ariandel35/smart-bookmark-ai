...
- 新增快速/完整速度模式：快速模式跳过失效链接检测、额外目录规划请求和模型等待，完整模式保留链接检查和 AI 分类，并按服务商速度决定是否额外规划目录
...
- Added Fast and Complete speed modes: Fast skips dead-link checks, the extra taxonomy-planning request, and model waiting, while Complete keeps link checks and AI classification with provider-aware planning
