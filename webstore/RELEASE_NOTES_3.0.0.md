...
- DeepSeek 等慢响应模型会在第一次请求前使用更安全的运行批大小，超时后还会继续降批重试
...
3.0.0 优化了核心使用路径：先预览，再应用方案。应用预览不会再次跑模型；快速模式默认更快且权限更少，并少一次单独目录规划请求，规则和缓存覆盖全部书签时还会直接本地完成；设置页使用内联确认和校验提示，API 检测成功后会保存连接配置，DeepSeek 等慢模型会先使用更安全批量并在超时后自动降批重试。
...
- Slow providers such as DeepSeek start with a safer runtime batch size and still retry with smaller batches after timeouts
...
3.0.0 streamlines the core workflow: preview first, then apply the plan without a second model run. Fast mode skips dead-link checks and the extra taxonomy-planning request for quicker runs and fewer permissions, and now finishes locally when rules and cached classifications cover every bookmark. Settings use inline confirmations and validation feedback, successful API tests save the connection, and slow providers such as DeepSeek start with safer runtime batches before timeout retries shrink again.
