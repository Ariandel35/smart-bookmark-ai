...
- [ ] 如需自动化真实扩展 E2E，运行 `npm run e2e:extension`，使用允许 `--load-extension` 的 Chrome for Testing 或 Chromium；自动化默认 headless 后台运行，只有调试时才设置 `MARKO_SHOW_BROWSER=1`；并确认 Marko 的 service worker 是 `background.js`，临时书签可完成真实弹窗预览/应用点击流、真实弹窗未处理项删除点击流、真实设置页备份创建/恢复/删除点击流、手动备份、快速预览、应用方案、重复清理、备份记录、真实设置页保存和 DeepSeek 批量压低验证
...
- [ ] 设置页在 390px、720px、1280px 宽度下没有横向滚动，连接区、备份区、长错误提示和长服务商/模型值不会撑宽页面
